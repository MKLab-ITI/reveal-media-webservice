 ...
 * 
 */
public class ExtendedGraphMLWriter<V,E> 
{
	...
	public void save(Hypergraph<V,E> graph, Writer w) throws IOException
	{
		...
		for (String key : graph_data.keySet()) {
			writeKeySpecification(key, "graph", graph_data.get(key), null, bw);
		}
		for (String key : vertex_data.keySet()) {
			GraphMLMetadata<V> metadata = vertex_data.get(key);
			writeKeySpecification(key, "node", metadata, metadata.description, bw);
		}
		for (String key : edge_data.keySet()) {
			GraphMLMetadata<E> metadata = edge_data.get(key);
			writeKeySpecification(key, "edge", metadata, metadata.description, bw);
		}
...
//	}
	...
	protected void writeKeySpecification(String key, String type, 
			GraphMLMetadata<?> ds, String attrType, BufferedWriter bw) throws IOException
	{
		bw.write("<key id=\"" + key + "\" for=\"" + type + "\"");
		
		if(attrType != null) {
			bw.write(" attr.type=\"" + attrType + "\"");
		}
