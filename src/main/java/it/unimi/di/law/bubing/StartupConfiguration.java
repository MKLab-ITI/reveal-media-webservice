 ...
 * <samp>K</samp> (10<sup>3</sup>), <samp>Ki</samp> (2<sup>10</sup>), <samp>M</samp> (10<sup>6</sup>), <samp>Mi</samp> (2<sup>20</sup>), etc. 
 * 
 ...
 * <h2>Internals</h2>
 * 
 * <p>If a field named <samp>xxYyyZzzz</samp> requires some additional check (e.g., that its value
...
public class StartupConfiguration {
	 ...
	 * to use suffixes <samp>ms</samp> (milliseconds), <samp>s</samp> (seconds),  <samp>m</samp> (minutes),  <samp>h</samp> (hours) and  <samp>d</samp> (days). */ 
	@Retention(RetentionPolicy.RUNTIME)
	@Target(ElementType.FIELD)
	...
	/** A URL from which BUbiNG will start crawling. If it starts with <samp>file:</samp>,
	...
	/** A host that should be blacklisted (i.e., not crawled). If it starts with <samp>file:</samp>,
	...
	/** The From header used for HTTP requests. It can be empty, in which case no <samp>From</samp> header will be emitted. */
	@OptionalSpecification(value="")
	public String userAgentFrom;

	/** The delay after which the <samp>robots.txt</samp> file is no longer considered valid. */
	@TimeSpecification
	 ...
	 *   corresponding <samp>check...</samp> method.
