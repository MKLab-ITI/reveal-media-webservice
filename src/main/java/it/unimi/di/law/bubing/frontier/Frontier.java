...
public class Frontier implements JobListener<BubingJob>, AbstractSieve.NewFlowReceiver<ByteArrayList> {
	...
	/** The Warc file where to write (if so requested) the downloaded <samp>robots.txt</samp> files. */
	...
	/** The number of fetched <samp>robots.txt</samp> files (updated by {@link ParsingThread}
	 * instances). */
	...
	/** The default configuration for a non-<samp>robots.txt</samp> request. */
	public final RequestConfig defaultRequestConfig;
	/** The default configuration for a <samp>robots.txt</samp> request. */
	 ...
	 * single file named <samp>frontier.data</samp>. Other fields are written each in a file of its
	...
	public void snap() throws ConfigurationException, IllegalArgumentException, IOException {
		...
		for ( VisitState visitState : distributor.schemeAuthority2VisitState.visitStates() )
			if ( visitState != null && !visitState.isEmpty() && visitState.firstPath() == VisitState.ROBOTS_PATH ) {
		...
		LOGGER.info( "Storing virtualizer states and freezing virtual queues" );
		// TODO what should we do here? the commented-out part is needed by the queue
		// virtualization, not by the BDB implementation
		/* scalarData.addProperty( PropertyKeys.CURRENTQUEUE, virtualizer.currentQueue ); final int
		 * numVirtualQueues = virtualizer.virtualQueue.length; String[] sizes = new String[
		 * numVirtualQueues ]; for( int i = 0; i < numVirtualQueues; i++ ) { final
		 * ByteArrayDiskQueue q = virtualizer.virtualQueue[ i ]; sizes[ i ] = Long.toString( q ==
		 * null? -1 : q.size64() ); if ( q != null ) q.freeze(); } scalarData.addProperties(
		 * PropertyKeys.VIRTUALQUEUESIZES, sizes ); scalarData.addProperty(
		 * PropertyKeys.VIRTUALQUEUESBIRTHTIME, virtualizer.virtualQueuesBirthTime ); */
		virtualizer.close();
	...
	public void restore() throws ConfigurationException, IllegalArgumentException, IOException, ClassNotFoundException, InterruptedException {
		...
		for ( long workbenchSize = workbenchStream.readLong(); workbenchSize-- != 0; ) {
