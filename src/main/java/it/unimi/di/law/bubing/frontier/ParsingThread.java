 ...
 * that it is respecting the <samp>robots.txt</samp> protocol, and <em>this behavior cannot be
...
public class ParsingThread extends Thread {
    ...
    protected static final class FrontierEnqueuer {
        ...
        private static boolean sameSchemeAuthority(final byte[] schemeAuthority, final URI url) {
            ...
            for (int i = authority.length(); i-- != 0; )
                if (schemeAuthority[schemeLength + i] != (byte) authority.charAt(i)) return false;
            return true;
        }
            ...
            for (; ; ) {
                ...
                try { // This try/finally guarantees that we will release the visit state and signal back.
                    ...
                    if (fetchData.exception != null) {
                        ...
                        if (visitState.lastExceptionClass != exceptionClass) {
                            ...
                            if (visitState.lastExceptionClass == null) {
                                ...
                                //frontier.limbo.remove( visitState );
                            }
                                        ...
                                        try {
                                            ...
                                            if (rc.spamDetector != null && (visitState.termCountUpdates < rc.spamDetectionThreshold || rc.spamDetectionPeriodicity != Integer.MAX_VALUE)) {
                                                ...
                                                if (result instanceof SpamTextProcessor.TermCount)
                                                    visitState.updateTermCount((SpamTextProcessor.TermCount) result);
                    ...
                    if (digest == null) {
                        ...
                        if (streamLength != 0 && LOGGER.isDebugEnabled())
                            LOGGER.debug("Computing binary digest for " + url);
                    ...
                    if (LOGGER.isTraceEnabled())
                        LOGGER.trace("Decided that for {} isNotDuplicate={}", url, Boolean.valueOf(isNotDuplicate));
                        ...
                        if (isNotDuplicate) {
                            ...
                            if (contentTypeHeader != null) {
                                ...
                                if (StringUtils.startsWithIgnoreCase(contentType, "text"))
                                    frontier.contentTypeText.incrementAndGet();
                                else if (StringUtils.startsWithIgnoreCase(contentType, "image"))
                                    frontier.contentTypeImage.incrementAndGet();
                                else if (StringUtils.startsWithIgnoreCase(contentType, "application"))
                                    frontier.contentTypeApplication.incrementAndGet();
