...
public final class StatsThread implements Runnable {
	...
	public StatsThread( final Frontier frontier, final Distributor distributor ) {
		...
		receivedURLsLogger = new ProgressLogger( LOGGER, Long.MAX_VALUE, TimeUnit.MILLISECONDS, "URLs" );
	...
	public void emit() {
		...
		LOGGER.info( "Duplicates: " + duplicates + " (" + Util.format( 100.0 * duplicates / ( duplicates + archetypes ) ) + "%)" );
		...
		LOGGER.info( "Ready URLs: " + frontier.readyURLs.size64() );
