...
public class VisitState implements Delayed, Serializable {
	...
	/** A special path marking a <samp>robots.txt</samp> refresh request. */
	...
	public VisitState( final Frontier frontier, final byte[] schemeAuthority ) {
		...
		termCount = frontier.rc.spamDetector == null ? null : new Short2ShortOpenHashMap();
		spammicity = -1;
	}
	...
	/** Enqueues the <samp>/robots.txt</samp> path as the first element of the queue, if the queue is
	...
	/** Forcibly enqueues the <samp>/robots.txt</samp> path as the first element of the queue.
	 *
	...
	/** Checks whether the current robots information has expired and, if necessary, schedules a new <samp>robots.txt</samp> download.
	 * 
	 * @param time the current time.
	 */
