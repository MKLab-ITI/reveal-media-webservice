...
public class Workbench implements Iterable<WorkbenchEntry> {
	...
	public void release( final VisitState visitState ) {
		...
		// TODO: we could avoid limbo removal here, and do it at purge time
		// ALERT: temporarily removed
		/*
		 if ( visitState.nextFetch == Long.MAX_VALUE ) limbo.remove( visitState );
		 else if ( visitState.size() < LIMBO_BOUND && visitState.urlsOnDisk != 0 && visitState.lastExceptionClass == null ) limbo.put( visitState, Boolean.TRUE ); 
		 */
	}
