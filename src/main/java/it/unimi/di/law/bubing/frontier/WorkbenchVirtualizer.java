...
public class WorkbenchVirtualizer implements Closeable {
	...
	public void readMetadata() throws IOException, ClassNotFoundException {
		...
		for( int i = ois.readInt(); i-- != 0; ) {
			...
			Distributor distributor = frontier.distributor;
			VisitStateSet schemeAuthority2VisitState = distributor.schemeAuthority2VisitState;
			byteArrayDiskQueues.key2QueueData.put( schemeAuthority2VisitState.get( schemeAuthority, 0, length ), (QueueData)ois.readObject() );
		}
