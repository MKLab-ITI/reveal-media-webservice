 ...
 *  	based on the declared <samp>content-type</samp> header etc.);
...
public interface Parser<T> extends Filter<URIResponse> {
	...
	public static interface LinkReceiver extends Iterable<URI> {
		...
		/**
		 * Handles the location defined by a <samp>META</samp> element.
		 * 
		 * @param location the location defined by the <samp>META</samp> element.
		 */
		public void metaLocation( URI location );

		/**
		 * Handles the refresh defined by a <samp>META</samp> element.
		 * 
		 * @param refresh the URL defined by the <samp>META</samp> element.
		 */
